...
我们为 Docker 用户专门开发了交互式管理工具，无需手动修改配置文件，无需记忆复杂命令。
...
**运行后你将看到交互式菜单，支持以下所有操作：**

- **📺 监控平台管理**：像点菜一样开启/关闭/添加监控平台（支持自动补全名称）
- **📝 关键词管理**：分组管理关键词，支持增删改
- **⏱️ 抓取频率设置**：可视化修改 Cron 表达式 (支持 6/8/12 小时或每天固定时间)，**重启容器依然有效**
- **🔔 推送通知配置**：向导式配置飞书、钉钉、微信、邮件等推送通道
- **▶️ 立即运行**：手动触发一次抓取任务

> 💡 **提示**：这是最适合"小白"的管理方式，完全可视化的操作体验。
